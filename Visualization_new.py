...
def visualize(data=dates, zip_name=['Facility_3_100','Facility_4_100','Facility_6_100'], type='CSs'):
    set_style()
    ...
    if type == 'Energy':
        ...
        for i in range(0, 5):
            vehicles_summer_3[i] = pd.read_csv(zf_3.open(f'vehicles_{facility_3}_{data[i]}.csv'))
            vehicles_summer_4[i] = pd.read_csv(zf_4.open(f'vehicles_{facility_4}_{data[i]}.csv'))
            vehicles_summer_6[i] = pd.read_csv(zf_6.open(f'vehicles_{facility_6}_{data[i]}.csv'))
            ...
            dgs_energy_summer_3[i] = vehicles_summer_3[i].groupby(['time']).agg({'Energy': 'sum'}).reset_index()
            dgs_energy_summer_3[i]['day'] = data[i]
            df_energy_summer_3 = df_energy_summer_3.append(dgs_energy_summer_3[i])

            dgs_energy_summer_4[i] = vehicles_summer_4[i].groupby(['time']).agg({'Energy': 'sum'}).reset_index()
            dgs_energy_summer_4[i]['day'] = data[i]
            df_energy_summer_4 = df_energy_summer_4.append(dgs_energy_summer_4[i])

            dgs_energy_summer_6[i] = vehicles_summer_6[i].groupby(['time']).agg({'Energy': 'sum'}).reset_index()
        ...
        for i in range(5, 10):
            vehicles_winter_3[i] = pd.read_csv(zf_3.open(f'vehicles_{facility_3}_{data[i]}.csv'))
            vehicles_winter_4[i] = pd.read_csv(zf_4.open(f'vehicles_{facility_4}_{data[i]}.csv'))
            vehicles_winter_6[i] = pd.read_csv(zf_6.open(f'vehicles_{facility_6}_{data[i]}.csv'))
            ...
            dgs_energy_winter_3[i] = vehicles_winter_3[i].groupby(['time']).agg({'Energy': 'sum'}).reset_index()
            dgs_energy_winter_3[i]['day'] = data[i]
            df_energy_winter_3 = df_energy_winter_3.append(dgs_energy_winter_3[i])

            dgs_energy_winter_4[i] = vehicles_winter_4[i].groupby(['time']).agg({'Energy': 'sum'}).reset_index()
            dgs_energy_winter_4[i]['day'] = data[i]
            df_energy_winter_4 = df_energy_winter_4.append(dgs_energy_winter_4[i])

            dgs_energy_winter_6[i] = vehicles_winter_6[i].groupby(['time']).agg({'Energy': 'sum'}).reset_index()
        ...
        ax[0].set_xlabel('Time of day', fontsize=16)
        ax[0].set_ylabel('Energy (KWh)', fontsize=16)
        ax[0].set_title('Energy consumption in summer days', fontsize=16)

        ax[1].set_xlim((0, 24))
        ax[1].set_ylim((0, 1200))
        ax[1].set_xlabel('Time of day', fontsize=16)
        ax[1].set_ylabel('Energy (KWh)', fontsize=16)
        ax[1].set_title('Energy consumption in winter days', fontsize=16)




        '''sns.lineplot(data=dg_energy_summer_3, x='time', y='Energy', ci=95, legend=True, ax=ax[0], label='Mixed-use')
        sns.lineplot(data=dg_energy_winter_3, x='time', y='Energy', ci=95, legend=True, ax=ax[1], label='Mixed-use')
        sns.lineplot(data=dg_energy_summer_4, x='time', y='Energy', ci=95, legend=True, ax=ax[0], label='Destination')
        sns.lineplot(data=dg_energy_winter_4, x='time', y='Energy', ci=95, legend=True, ax=ax[1], label='Destination')
        sns.lineplot(data=dg_energy_summer_6, x='time', y='Energy', ci=95, legend=True, ax=ax[0], label='Workplace')
        sns.lineplot(data=dg_energy_winter_6, x='time', y='Energy', ci=95, legend=True, ax=ax[1], label='Workplace')'''
    ...
    if type == 'CSs':
        ...
        sns.boxplot(data=costs[costs['Season']=='Summer'], x='Facility Title', y='NoC', ax=ax[0])
        sns.boxplot(data=costs[costs['Season']=='Summer'], x='Facility Title', y='P_plus', ax=ax[1])
        # ax[0].set_xlim((0, 24))
        # ax[0].set_ylim((0, 1500))
        ax[0].set_xlabel('Facility Type', fontsize=16)
        ax[0].set_ylabel('Number', fontsize=16)
        ax[0].set_title('Number of EVSEs', fontsize=16)

        # ax[1].set_xlim((0, 24))
        #ax[1].set_ylim((300, 370))
        ax[1].set_xlabel('Facility Type', fontsize=16)
    ...
    if type == 'Benchmark':
        ...
        '''sns.boxplot(data=costs, x='Facility Title', y='NoC', hue='Season', ax=ax[0])
        sns.boxplot(data=costs, x='Facility Title', y='NoP', hue='Season', ax=ax[1])'''
        sns.barplot(data=df[df['Season'] == 'Summer'], x='Adoption', y='NoC', hue= 'Solution', ax=ax[0][0])
        sns.barplot(data=df[df['Season'] == 'Summer'], x='Adoption', y='P_plus', hue='Solution',  ax=ax[0][1])
        sns.barplot(data=df[df['Season'] == 'Summer'], x='Adoption', y='Installation_costs', hue='Solution', ax=ax[1][0])
        sns.barplot(data=df[df['Season'] == 'Summer'], x='Adoption', y='Operations_costs', hue='Solution', ax=ax[1][1])
        # ax[0].set_xlim((0, 24))
        # ax[0].set_ylim((0, 1500))
        ax[0][0].set_xlabel('EV adoption rate', fontsize=16)
        ax[0][0].set_ylabel('Number', fontsize=16)
        ax[0][0].set_title('Number of EVSEs', fontsize=16)

        # ax[1].set_xlim((0, 24))
        # ax[1].set_ylim((0, 1500))
        ax[0][1].set_xlabel('EV adoption rate', fontsize=16)
        ax[0][1].set_ylabel('Power (KW)', fontsize=16)
        ax[0][1].set_title('Grid expansion', fontsize=16)

        ax[1][0].set_xlabel('EV adoption rate', fontsize=16)
        ax[1][0].set_ylabel('Cost (USD)', fontsize=16)
        ax[1][0].set_title('Investment cost', fontsize=16)

        # ax[1].set_xlim((0, 24))
        # ax[1].set_ylim((0, 1500))
        ax[1][1].set_xlabel('EV adoption rate', fontsize=16)
        ax[1][1].set_ylabel('Cost (USD)', fontsize=16)
        ax[1][1].set_title('Daily operations cost', fontsize=16)
        plt.tight_layout()
    ...
    if type == 'Utilization':
        ...
        for i in range(0, 5):
            for p in [1, 2, 4, 6]:
                ...
                costs.loc[(costs['Date'] == data[i]) & (costs['Plugs'] == p), 'Utilization'] = df.groupby(['time'])\
                    .agg({'Energy': 'sum'}).reset_index().mean()['Energy']/(costs.loc[(costs['Date'] == data[i])&
                                                                                     (costs['Plugs'] == p), 'NoC'] * 22)
        costs['P_plus'] = costs['P_plus'] * 1.25
        df = costs
        fig, ax = plt.subplots(1, 2, figsize=(16, 8))
        sns.barplot(data=df[df['Season'] == 'Summer'], x='Plugs', y='Utilization', ax=ax[0])
