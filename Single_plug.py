...
def planning(facility, date):
    ...
    results.loc[(results['Plugs'] == 1) & (results['Date'] == date), 'Installation_costs'] = c1.solution_value
    results.loc[(results['Plugs'] == 1) & (results['Date'] == date), 'Operations_costs'] = c2.solution_value
    results.loc[(results['Plugs'] == 1) & (results['Date'] == date), 'NoC'] = NoC.solution_value
    results.loc[(results['Plugs'] == 1) & (results['Date'] == date), 'NoP'] = NoC.solution_value
    results.loc[(results['Plugs'] == 1) & (results['Date'] == date), 'P_plus'] = p_plus.solution_value
    results.loc[(results['Plugs'] == 1) & (results['Date'] == date), 'P_star'] = p_star.solution_value
    results.loc[(results['Plugs'] == 1) & (results['Date'] == date), 'costs'] = mdl.objective_value
    results.loc[(results['Plugs'] == 1) & (results['Date'] == date), 'Event'] = dfs["EntryHour"].count()
